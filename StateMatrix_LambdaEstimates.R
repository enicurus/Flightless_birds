...
for (i in(1:1000)){JGF[[i]]$edge.length<-abs(JGF[[i]]$edge.length)}
write.nexus(JGF,file="/Users/ryanterrill/Dropbox/Flightless_project/Trees/All_Birds/JetzGeneFlightless/JGF_final.nexus")
...
for (i in(1:1000)){JAF[[i]]$edge.length<-abs(JAF[[i]]$edge.length)}
